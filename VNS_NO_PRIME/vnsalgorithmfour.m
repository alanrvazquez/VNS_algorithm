function [ best_cdes, objvalue ] = vnsalgorithmfour( parents, BB, objvalue, n, d, m, sset, nset, possibJhc, vecbigm )
% VNS Algorithm for improving the J4-characteristics of a concatenated design.
% Version for parent designs with a number of basic factors that is NOT a prime
%
% INPUTS:
...
% BB        The (n*d)-by-m matrix design formed by concatenating d copies of
%           the basic factor columns of the regular design.
...
while ii <= (d-1) 
    ...
    for f = factors    
        ...
        cdes = concatenate(parents, n, m, d); % Concatenate parents.---
        combined_design = [BB, cdes]; % Build entire design.---------------
        % Evaluate design.-------------------------------------------------
        resvalue = objective(combined_design, sset, nset, possibJhc, vecbigm);
        
        if resvalue < objvalue 
               objvalue = resvalue; 
               best_cdes = combined_design; 
               ii = 1; % Go back to N1.------------------------------------
               break 
        end
